...
if [ $# -ne 3 ]; then
    echo "Usage: $0 <READSETPFX> <METADATA> <OUTDIR>"
    exit 1
fi
READSETPFX=$1
BUCKETS=$2
...
if [ ! -e $INDEX.1.cf ]; then
    echo "Error: $INDEX.1.cf was not found" > /dev/stderr
    exit 1
fi

if [ ! -e $TAXDUMP/nodes.dmp ]; then
    echo "Error: $TAXDUMP/nodes.dmp was not found" > /dev/stderr
    exit 1
fi

if [ ! -e $BUCKETS ]; then
    echo "Error: $BUCKETS was not found" > /dev/stderr
    exit 1
fi
