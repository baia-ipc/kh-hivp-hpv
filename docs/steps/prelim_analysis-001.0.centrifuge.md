# prelim_analysis step 001: Centrifuge bucketing
...
- `bin/prelim_analysis.steps/001.0.centrifuge.run.sh` (all samples)
- `bin/prelim_analysis.steps/single_sample/001.0.centrifuge.run_sample.sh` (one sample / one pair)
...
- Optional: build a local index with `scripts/centrifuge_bucketing/build_centrifuge_db.sh`
...
- The same pipelines are also reused for `targeted_analysis/001.0.bucketing/` with a
