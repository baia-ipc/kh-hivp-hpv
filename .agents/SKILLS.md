...
- Inputs: `metadata/samples_prelim_analysis.tsv` or `metadata/samples_targeted_analysis.tsv` (normalized `sample_id` + raw `fastq_sample_id`) plus Centrifuge index/taxdump.
- Outputs:
...
- Inputs: curated tree inputs under `derived_data/refdata/hpv16_tree` and `derived_data/refdata/hpv18_tree` (prepared from `refdata/hpv16_tree` and `refdata/hpv18_tree`, plus accession lists in `metadata/hpv16_tree_db_selection.txt` and `metadata/hpv18_tree_db_selection.txt`).
- Outgroups are listed in `metadata/hpv16_tree_outgroups.txt` / `metadata/hpv18_tree_outgroups.txt` and extracted from the PAVE FASTA.
...
- Lineage annotation helpers read `metadata/hpv16_lineage_refs.tsv` and `metadata/hpv18_lineage_refs.tsv` to assign lineages to tree tips when needed.
