...
- `metadata/`: `bucketing/`, `cohort/`, `hpv16_tree/`, `hpv18_tree/`,
  `recombination_analysis/`, `seq_samples/`.
- `config/`:
...
- `targeted_analysis`:
  - `01.bucketing` -> `02.mapping_vs_pave` -> `03.variant_analysis` ->
    `04.hpv16_tree` and `05.hpv18_tree`; `06.recombination_analysis` consumes a
    metadata-provided FASTA set.
...
- Metadata is organized in subdirectories:
  ...
  `metadata/hpv18_tree`, `metadata/recombination_analysis`, and
  `metadata/seq_samples`.
...
- Lineage reference tip TSVs are generated by the phylo tree pipeline and
  written under `derived_data/refdata/hpv16_tree/` and
...
- Database comparison is part of step `03.variant_analysis` (not a separate
  standalone step).
- Recombination screening step `06.recombination_analysis` uses metadata-driven
  FASTA input plus MAFFT + HyPhy GARD; RDP is not automated in-pipeline.
